    ...
    public double checkPriceOnAmazon (String isbn13) {
        ...
        try {
            ...
            try {
                        ...
                        if (button.toLowerCase().contains("$")) {
                            if (button.toLowerCase().contains("from")) {
                                if (button.toLowerCase().contains("perfect")) {
                                    ...
                                    driver.quit();
                                    return convertPrice(amazonPrice);
                                } else {
                ...
                driver.quit();
                System.out.println("Could not find buttons with prices");
                return 0.00;
            }
        } catch (NoSuchElementException b) {
            driver.quit();
            System.out.println("Could not find title with link");
            return 0.00;
        }
    }

    public double convertPrice(double priceUSDEUR) {
